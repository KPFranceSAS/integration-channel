    ...
    protected function postUpdateStatusDelivery(WebOrder $order, $invoice, $trackingNumber)
    {
            ...
            if ($result) {
                ...
                if ($orderAliexpress->logistics_status == "WAIT_SELLER_SEND_GOODS" && $orderAliexpress->order_status == "IN_CANCEL") {
                    $this->addOnlyErrorToOrderIfNotExists($order, 'Error posting tracking number ' . $trackingNumber . ' Customer asks for cancelation and no response was done online. A response should be brought before ' . DatetimeUtils::createDateTimeFromDateWithDelay($orderAliexpress->over_time_left));
                } elseif ($orderAliexpress->order_status == 'FINISH' && $orderAliexpress->order_end_reason == "cancel_order_close_trade") {
                    $this->addOnlyErrorToOrderIfNotExists($order, 'Error posting tracking number ' . $trackingNumber . ' Order has been cancelled online on ' . DatetimeUtils::createDateTimeFromDateWithDelay($orderAliexpress->gmt_trade_end));
                    ...
                    $this->addOnlyErrorToOrderIfNotExists($order, 'Error posting tracking number ' . $trackingNumber . ' Order has been cancelled online because delay of expedition is out of delay on ' . DatetimeUtils::createDateTimeFromDateWithDelay($orderAliexpress->gmt_trade_end));
                    $order->setStatus(WebOrder::STATE_CANCELLED);
                } else {
